...
def add_patient():
    ...
    def add():
        ...
        if not NAME.isalpha():
            messagebox.showinfo("Error", "Name should contain only alphabets")
            return
        ...
        if not MOBILE.isdigit() or len(MOBILE) != 10:
            ...
            return
        # Check if DOB is in the valid format (you may need to modify this condition based on the expected format)
        if not validate_date_format(DOB):
            messagebox.showinfo("Error", "Invalid Date of Birth format. Please use YYYY-MM-DD")
            return

        # Check if Medical History contains only alphabets and digits
        if not HISTORY.isalpha():
            messagebox.showinfo("Error", "Medical History should contain only alphabets and digits")
            return
         # Check if a patient with the same name and mobile number already exists
        for patient in read():
        ...
        try:
            ...
            conn.commit()
            conn.close()
            success_label.config(text=f"Patient added successfully\nDetails: MOBILE: {MOBILE}, NAME: {NAME}, DOB: {DOB}, HISTORY: {HISTORY}, MEDICINES: {MEDICINES}")
    ...
    refreshTable()
     # New label to display added or updated patient details
    success_label = Label(root, text="", font=('Arial', 15))
    success_label.grid(row=18, column=0, columnspan=10, padx=50, pady=10)
